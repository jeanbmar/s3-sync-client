...
const util = {
    ...
    diff(sourceObjects, targetObjects, sizeOnly = false) {
        const sourceObjectMap = new Map(
            Array.from(sourceObjects.values())
                .map((sourceObject) => [sourceObject.id, sourceObject]),
        );
        const targetObjectMap = new Map(
            Array.from(targetObjects.values())
                .map((targetObject) => [targetObject.id, targetObject]),
        );
        const created = [];
        const updated = [];
